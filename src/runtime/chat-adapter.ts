...
import type { ChatMessage, ChatTurnResult, ChatMode, DraftCanonicalState, DraftTask } from '../domain/chat.js';
...
- draftTask: optionally suggest 1-2 starter tasks if you have enough context. Keep these lightweight.
- Do not invent information the user hasn't mentioned or implied`;
}

function buildSteeringSystemPrompt(stateSummary: string): string {
...
Field schemas:
- draftState (kickoff mode): { "goal": "", "phase": "", "phaseGoal": "", "constraints": [], "decisions": [], "risks": [], "knownFiles": [], "nextStep": "" } — include all fields you know, null/omit unknown ones
- draftTask: { "title": "", "goal": "", "taskType": "discovery"|"implementation"|"validation", "scopePaths": [] }
- Set unused fields to null`;
}

// ── Context building ──

function buildStateSummary(state: CanonicalProjectState): string {
  const parts: string[] = [];
...
export async function sendChatTurn(
  ...
  if (mode === 'kickoff') {
    ...
    sections.push(buildSteeringSystemPrompt(stateSummary));
  }
